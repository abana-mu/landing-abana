...
<template>
  <div class="">
    <div v-for="(row, index) in data" :key="index" :id="index" class="two-col ">
      <div class="contain grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 md:gap-4 lg:gap-10 py-16 lg:py-24">
        <div class="mb-2 md:mb-0 lg:col-span-5 two-col-text">
          <div class="p-0">
            <div class="mb-5">
              <h3 class="text-3xl lg:text-4xl font-medium">{{ row.title }}</h3>
              <p class="italic text-subtitle" v-if="row.subtext">{{ row.subtext }}</p>
            </div>
            <vue-simple-markdown :source="row.content" class="mb-8 text-body"></vue-simple-markdown>
            <div v-if="row.learnmore">
              <LearnMore :source="row.learnmore" />
            </div>
            <g-link class="group inline-flex items-center text-primary hover:underline" v-if="row.url" :to="row.url">
              {{ row.linkText }}
              <!-- Arrow right-short -->
