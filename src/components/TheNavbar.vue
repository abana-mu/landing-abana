...
<template>
  <div>
    <div
      ...
      class=" bg-primary z-20 relative flex w-full top-0 justify-center items-center text-center h-10"
    >
      <a :href="link" target="_blank" class="text-white text-sm sm:text-base hover:underline">{{ Data.text }}</a>
    </div>
    <div class="relative">
      <div :class="[{ 'nav-sticky': scrolled }, Data.active && !scrolled ? 'top-10' : 'top-0']" class="transition duration-1000 z-10 fixed w-full">
        <div class="mx-auto max-w-7xl lg:px-5 lg:py-4">
          <div
            class="nav-inner relative px-4 xs:px-5 sm:px-6 lg:px-5 py-4 lg:py-0 flex justify-between bg-white items-center space-x-6 xl:space-x-10 z-20"
          >
            <div class="flex flex-row items-center space-x-10">
              <!-- Logo -->
              <div class="flex flex-col lg:flex-1">
                <g-link class="nav-logo w-28 xs:w-36 h-auto block relative pb-4 sm:pb-5 lg:w-36 transition" to="/">
              ...
              <nav class="space-x-4 hidden lg:flex">
                <div
                  ...
                  class="nav-link md:px-2 xl:px-4 py-6 group transition"
                  :key="index"
                  ...
                  <div v-if="link.drop">
                    ...
                    >
                      <div v-if="activetab === index" class="absolute top-full transform w-3/4 lg:left-1/2 lg:-translate-x-1/2 origin-top pt-3">
                        <div class="dropdown-style border rounded-lg overflow-hidden max-w-3xl">
                          <div v-if="link.drop.type === 1" class="relative grid gap-6 bg-white sm:gap-8 lg:grid-cols-5">
                            <div class="col-span-3 p-5">
                              <g-link
                                :to="drop.link"
                                class="drop-item p-5 flex items-center space-x-4"
                                v-for="(drop, index) in link.drop.links"
                                :key="index"
                              >
                                <component :is="drop.icon" class="drop-item-icon h-12" />
                                <div class="flex flex-col">
                                  <span class="drop-item-title font-medium text-title text-lg">{{ drop.title }}</span>
                                  <span class="drop-item-subtitle  text-caption text-xs ">{{ drop.title }}</span>
                                </div>
                              </g-link>
                            </div>
                            <div class="col-span-2 bg-gray-50 p-5">
                              <g-link
                                :to="drop.link"
                                class="p-2 flex items-center space-x-2 cursor-pointer"
                                v-for="(drop, index) in link.drop.extra"
                                :key="index"
                              >
                                <component :is="drop.icon" class="h-5" />
                                <div class="flex flex-col">
                                  <span class="font-regular text-caption hover:text-body text-base">{{ drop.title }}</span>
                                </div>
                              </g-link>
                            </div>
                          </div>
            ...
            <!-- Right Hand Buttons -->
            <div class="hidden lg:flex items-center justify-end md:flex-1 lg:w-0 divide-x-2">
                  ...
                  >
                    <div class="contain pt-5">
                      ...
                      <div v-for="(link, index) in link.drop.links" :key="index">
                        <div class="w-full py-4 inline-flex justify-between items-center font-medium text-body hover:text-primary cursor-pointer">
...
.nav-sticky {
  @apply transition duration-300 bg-white lg:bg-transparent;

  .nav-inner {
    @apply py-6 lg:py-0 lg:rounded-xl;
